...
function haptic(ms=12){try{if('vibrate' in navigator) (navigator as any).vibrate(ms);}catch{}}
...
function Home(){
  ...
  const filtered=useMemo(()=>{
    ...
    if(!q) return list;
    return list.filter(n=>n.title.toLowerCase().includes(q)||(n.tags||[]).some(t=>t.toLowerCase().includes(q))||n.category.toLowerCase().includes(q));
  ...
  useEffect(()=>{const onR=()=>setVp({w:window.innerWidth,h:window.innerHeight});window.addEventListener('resize',onR);return()=>window.removeEventListener('resize',onR);},[]);
  ...
  const focusNode=(n:Node,openPanel=true)=>{
    ...
    saveLastVisited(n.id); setResumeId(n.id);
    setCrumbs(pushCrumb(n.id));
    if(openPanel) navigate(`/topic/${n.id}`);
  };

  const clearSelection=()=>{setSelected(null);navigate('/')};
